...
rc('font', **{'family': 'serif', 'serif': ['Computer Modern'], 'size': 24, 'weight': 'bold'})
rc('text', usetex=True)
rc('legend', fontsize=37)
...
# yfield = 'test_pos_int_traj'
# ylabel = '\\textbf{{Trajectory positional error (\% width)}}'
# val_scale = 100 / 2.0

# yfield = 'test_angle_int_traj'
# ylabel = '\\textbf{{Trajectory rotational error ($^{\circ}$)}}'
# val_scale = 57.3

yfield = 'test_penetration_int_traj'
ylabel = '\\textbf{{Trajectory penetration (\% width)}}'
val_scale = 100 / 2.0

# models = ['sweep-vert-.+', 'sweep-parallel-.+']
# label_lookup = {'sweep-vert-.+': 'Vertex', 'sweep-parallel-.+': 'Parallel'}
# color_lookup = {'sweep-vert-.+': '#01256e', 'sweep-parallel-.+': '#4a0042'}

models = ['sweep-e2e-.+', 'sweep-vert-.+', 'sweep-parallel-.+']
label_lookup = {'sweep-e2e-.+': 'End-to-end', 'sweep-vert-.+': 'ContactNets Polytope', 'sweep-parallel-.+': 'ContactNets Deep'}
color_lookup = {'sweep-e2e-.+': '#95001a', 'sweep-vert-.+': '#01256e', 'sweep-parallel-.+': '#4a0042'}

# models = ['sweep-e2e-.+', 'sweep-vert-.+', 'sweep-deepvert-.+', 'sweep-deepvert-smallweight-.+']
# label_lookup = {'sweep-e2e-.+': 'End-to-end', 'sweep-vert-.+': 'Vertex', 'sweep-deepvert-.+': 'Deepvert', 'sweep-deepvert-smallweight-.+': 'Smallweight'}
# color_lookup = {'sweep-e2e-.+': '#95001a', 'sweep-vert-.+': '#01256e', 'sweep-deepvert-.+': '#4a0042', 'sweep-deepvert-smallweight-.+': 'yellow'}
