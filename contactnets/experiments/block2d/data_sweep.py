...
def main(resume: bool, e2e: bool):
    command = 'tensorboard --samples_per_plugin images=0 --port 6007 --bind_all --logdir {} > /dev/null 2>&1'.format(dirs.results_path('sweep'))
    ...
    for i, run_num in enumerate(run_nums):
        ...
        def do_train(resume):
            if e2e:
                train.do_train_e2e(epochs=500, batch=1, patience=patience,
                        scheduler_step_size=scheduler_step_size, resume=resume)
            else:
                train.do_train_structured(epochs=500, batch=1, patience=patience,
                        scheduler_step_size=scheduler_step_size, resume=resume)
