...
from contactnets.entity import Entity, Dynamic3D, Ground3D
...
class ElasticLCP(InteractionResolver):
    ...
    def __init__(self, interactions: ModuleList,
                       G_bases: Tensor, restitution: Tensor, restitute_friction: Tensor) -> None:
        super(ElasticLCP, self).__init__(interactions, [])
    ...
    def compute_corner_impulses(self, sp, i=-1) -> Tensor:
        ...
        while torch.any(active_sims) > 0:
            (new_configs, new_velocities, new_impulse, new_phis) = \
                self.compute_impact_free_step(sp, configurations, \
                velocities, dts_left, active_contacts, active_sims, self.velocity_update_continuous)
            new_active_contacts = new_phis <= 0.
            delta_active_contact = (new_active_contacts) * (~active_contacts)

            impactors = delta_active_contact.sum(dim=1)
            (preimpact_dts, impacting_contacts) = self.calculate_impact_times(phis, new_phis, delta_active_contact, dts_left)
            ...
            (preimpact_configs, preimpact_velocities, preimpact_impulse, preimpact_phis) = \
                self.compute_impact_free_step(sp, configurations, \
                velocities, preimpact_dts, active_contacts, active_sims, self.velocity_update_continuous)
            ...
            (compression_configs, compression_velocities, compression_impulse, compression_phis) = \
                self.compute_impact_free_step(sp, preimpact_configs, \
                preimpact_velocities, 0.0*preimpact_dts, active_contacts, active_sims, self.velocity_update_null)
            ...
            restution_nominal_net_impulse = self.restitution_net_impulse(sp, compression_configs, compression_velocities, restution_nominal_impulses)

            restution_velocity = lambda s,dt,v: v + interaction.compute_M_i_history().bmm(restution_nominal_net_impulse)

            (rest_configs, rest_velocities, rest_impulse_correction, rest_phis) = \
                self.compute_impact_free_step(sp, compression_configs, \
                compression_velocities, 0.0*preimpact_dts, active_contacts, active_sims, restution_velocity)
            ...
            for i in range(batch_n):
                if impactors[i] > 0:
                    ...
                    new_impulse[i,:,:] += restution_nominal_impulses[i,:,:] + rest_impulse_correction[i,:,:]
        ...
        #sols = self.compute_impulse_lcp(lcp_mat, lcp_vec

        return (new_configs, new_velocities, total_impulse)

    def compute_impact_free_step(self, sp, configurations, velocities, dts, active_contacts, active_sims, velocity_update):
        ...
        E = tensor_utils.veceye(k, bases_n).unsqueeze(0).repeat(1, 1, 1)
        I = torch.eye(k).unsqueeze(0).repeat(1, 1, 1)
        ...
        for i in range(batch_n):
            if active_sims[i] and N_active_contact[i] > 0:
                (Mi, wi) = self.compute_lcp_step(sp, active_contacts[i,:], \
                    configurations[i:(i+1),:,:], velocities[i:(i+1),:,:], f, dts[i])
                ki = N_active_contact[i]
                sols = self.compute_impulse_lcp(Mi,wi)
                lambda_n = sols[:, 0:ki,:]
                lambda_t = sols[:, ki:ki + ki * bases_n,:]
                inflate = I[:,:,active_contacts[i,:]]
                lambda_n = inflate.bmm(lambda_n)
                #pdb.set_trace()
                # TODO: fix inflation
                #lambda_t = E.bmm(inflate.transpose(1,2)).bmm(lambda_t)
                lti = active_contacts[i,:].unsqueeze(1).repeat(1,bases_n).reshape(bases_n * interaction.k(), 1).squeeze()
                #if active_contacts.sum() > 1:
                #    pdb.set_trace()
                new_impulse_n[i,:] = lambda_n.view(-1)
                new_impulse_t[i,lti] = lambda_t.view(-1)

        new_impulse = torch.cat((new_impulse_n.unsqueeze(2), self.compute_G().bmm(new_impulse_t.unsqueeze(2))),dim=1)

        gamma = interaction.compute_gamma(configurations=[configurations, None])
        Jn_tilde = interaction.compute_Jn(configurations=[configurations, None])
        ...
        #pdb.set_trace()
        velocity_n = f + interaction.compute_M_i(configurations=[configurations, None]).bmm(net_impulse)
        configuration_n = configurations + interaction.compute_gamma(configurations=[configurations, None]).bmm(velocity_n).bmm(dts.reshape(batch_n,1,1))
    ...
    def compute_lcp_step(self, sp, active_contacts, configurations, velocities, f, dts):
        ...
        Efull = tensor_utils.veceye(interaction.k(), bases_n).unsqueeze(0).repeat(batch_n, 1, 1) > 0

        gamma = interaction.compute_gamma(configurations=[configurations, None])
        M_i = interaction.compute_M_i_history()
        #print(active_contacts)
        Jn = interaction.compute_Jn(configurations=[configurations, None])[:,active_contacts,:].bmm(gamma)
        #pdb.set_trace()
        doubled_ac = active_contacts.unsqueeze(1).repeat(1,bases_n).reshape(bases_n * interaction.k(), 1).squeeze()
        Jt = self.compute_Jt(configurations=configurations)[:,doubled_ac,:].bmm(gamma)
        interaction = self.interactions[0]


        LCP_vec = torch.cat((Jn.bmm(f), Jt.bmm(f),
    ...
    def compute_lcp_matrix(self, sp, active_contacts, configurations):
        ...
        gamma = interaction.compute_gamma(configurations=[configurations, None])
        M_i = interaction.compute_M_i_history()
        Jn_tilde = interaction.compute_Jn(configurations=[configurations, None])[:,active_contacts,:]
        Jn = Jn_tilde.bmm(gamma)
        doubled_ac = active_contacts.unsqueeze(1).repeat(1,bases_n).reshape(bases_n * interaction.k(), 1).squeeze()
