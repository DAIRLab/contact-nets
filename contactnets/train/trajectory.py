...
from contactnets.utils import utils, quaternion, system_io
...
def get_names(prediction: PredictionLoss, chop_fraction: float):
    ...
    names = ['pos_final_traj', 'angle_final_traj', 'pos_int_traj', 'angle_int_traj',
            'pos_bound_traj', 'angle_bound_traj', 'penetration_int_traj', 'penetration_max_traj']
...
class TrajectoryLoss(Loss):
    ...
    def __init__(self, system: System, prediction: PredictionLoss, chop_fraction = 0.0, ground_truth_phi = lambda x: torch.tensor(0)) -> None:
        super(TrajectoryLoss, self).__init__(get_names(prediction, chop_fraction),
                system, LossType.TRAJECTORY, True)
    ...
    def compute_travel_dist(self, config_hist: List[List[Tensor]], pos: bool) -> Tensor:
        if self.prediction.mode in [PredictionMode.XY, PredictionMode.XY_ROT]:
            dynamic_configs = [config[0] for config in config_hist]
            dynamic_configs = torch.cat(dynamic_configs, dim=2)
            
            configs_sel = dynamic_configs[:, 0:2, :] if pos else dynamic_configs[:, 2:3, :] 
    ...
    def compute_loss(self, meta_save_file: str = None) -> List[Tensor]:
        ...
        state_losses = []
        max_penetration = torch.tensor(0.)
        sum_penetration = torch.tensor(0.)
        ...
        if self.prediction.mode in [PredictionMode.XY, PredictionMode.XY_ROT]:
            ...
            final_ang_diff = quaternion.qdiff(final_config_data[:,3:7,:].squeeze(2),final_config_sim[:,3:7,:].squeeze(2))
            #pdb.set_trace()
        ...
        return [pos_error_percent.sum(), ang_error_percent.sum(), pos_sum,
                angle_sum, torch.tensor(pos_in_bound_n), torch.tensor(angle_in_bound_n), sum_penetration, max_penetration]
